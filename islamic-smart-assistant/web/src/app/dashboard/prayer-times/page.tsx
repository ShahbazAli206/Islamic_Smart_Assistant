...
export default function PrayerTimesPage() {
  ...
  const apply = () => {
    setCity(draftCity.trim() || 'Karachi');
    setCountry(draftCountry.trim() || 'Pakistan');
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-ink/60 mt-1">DST-aware, Hijri date included, multiple calculation methods.</p>
      </div>

      <PrayerCountdownHero city={city} country={country} />

      <div className="grid lg:grid-cols-3 gap-5">
        <div className="card card-pad lg:col-span-1">
          <h3 className="font-bold mb-2 flex items-center gap-2"><MapPin size={18} className="text-emerald-700"/> Set location</h3>
            ...
            {POPULAR.map((p, i) => {
              ...
              return (
                <motion.button
                  ...
                  whileHover={{ y: -2 }}
                  onClick={() => { setCity(p.city); setCountry(p.country); setDraftCity(p.city); setDraftCountry(p.country); }}
