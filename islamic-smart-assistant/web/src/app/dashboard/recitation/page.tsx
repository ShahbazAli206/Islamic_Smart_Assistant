  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div
        ...
        className="relative overflow-hidden rounded-3xl text-parchment shadow-glow-emerald"
        style={{ background: 'linear-gradient(125deg,#0d3d2c 0%,#0a3023 55%,#072018 100%)' }}
      >
        {/* faint mosque + pattern + drifting glows (all continuous) */}
        <div aria-hidden className="absolute inset-0 pointer-events-none">
          <div className="absolute inset-y-0 right-0 w-2/3 bg-cover bg-center opacity-[0.13]" style={{ backgroundImage: "url('/backgound-image2.png')" }} />
          <div className="absolute inset-0 pattern-bg opacity-[0.10]" />
          <div className="absolute -top-24 -right-10 w-72 h-72 rounded-full bg-glow-emerald animate-aurora" />
          <div className="absolute -bottom-28 left-1/4 w-72 h-72 rounded-full bg-glow-emerald opacity-50 animate-float-y" />
        </div>

        <div className="relative p-6 sm:p-8 grid lg:grid-cols-[1.3fr_1fr] gap-6 items-center">
          {/* left: title + CTAs */}
          <div>
            <motion.span
              animate={{ y: [0, -2, 0] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}
              className="inline-flex items-center gap-1.5 rounded-full border border-gold-300/40 bg-white/5 px-3 py-1.5 text-xs font-semibold text-gold-200 backdrop-blur"
            >
              <Bell size={12} /> Recitation Reminder
            </motion.span>
            <h1 className="mt-3 font-display font-bold text-4xl sm:text-5xl leading-none">Quran Recitation</h1>
            <p className="text-emerald-100/75 mt-3 max-w-md leading-relaxed">
              Let the words of Allah bring peace to your heart. Schedule your recitations and make Quran a part of your daily life.
            </p>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              <motion.button
                ...
                className="relative overflow-hidden inline-flex items-center gap-2 rounded-full bg-gold-gradient text-midnight-900 font-bold px-6 py-3 shadow-glow-gold"
              >
              ...
              <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 border border-white/15 px-3.5 py-2 text-sm font-medium">
                <CalendarClock size={14} className="text-gold-300" /> {schedules.length} Scheduled
              </span>
              <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 border border-white/15 px-3.5 py-2 text-sm font-medium">
                <span className="w-2 h-2 rounded-full bg-emerald-300 animate-pulse-soft" /> {activeCount > 0 ? 'Active' : 'Paused'}
              </span>
            </div>
          </div>

          {/* right: rotating ayah */}
          <div className="relative min-h-[150px] flex flex-col items-center lg:items-end justify-center text-center lg:text-right">
            <AnimatePresence mode="wait">
              ...
              >
                <p className="font-arabic text-2xl sm:text-3xl text-gold-200 leading-[1.9]" dir="rtl">{HERO_AYAT[heroIdx].ar}</p>
                <p className="mt-3 text-sm text-parchment/80 max-w-sm leading-relaxed">&ldquo;{HERO_AYAT[heroIdx].en}&rdquo;</p>
                <p className="mt-1.5 text-xs text-gold-300/80">({HERO_AYAT[heroIdx].ref})</p>
              </motion.div>
            </AnimatePresence>
